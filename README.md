...
> Status: security-conscious MVP. The local backup, history, verification, recovery, restore and explicit SFTP push/pull paths are implemented. Read [Security model and limits](#security-model-and-limits) before relying on it.
...
Tests cover age round trips, wrong keys, corruption, recovery, encrypted-manifest failure, path traversal, symlinks, collisions, Unix restore permissions, atomic replacement, incomplete backups, strict SSH host-key comparison and obvious secret leakage. Test values are fictitious.

## Known MVP limits

- Desktop recovery and SFTP configuration are currently documented in the Settings view but performed through the CLI; both use the same core and platform state.
